        ...
        <x-card>
            <form>
                ...
                <div class="mb-6" wire:ignore.self>
                    <label for="category_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kategori Barang</label>
                    <select wire:model="category_id" id="category_id" class="@error('category_id') is-invalid @enderror bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option value="null" disabled>{{ __('Please select') }}</option>
                        @foreach ($this->category as $item)
                            <option value="{{ $item->id }}" wire:key="cateogry-{{ $item->id }}">{{ $item->category }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        <span class="text-red-500">{{ $message }}</span>
